...
public class OthelloGameGUI extends GameGUI {
	...
	public OthelloGameGUI(String title, int width, int height,
		...
		m_backgroundTile = new ImageIcon(getClass().getResource(
				"OthelloBackground.png"));
		...
		m_whiteWinningPiece = new ImageIcon(getClass().getResource(
				"PieceWhiteFinal.png"));
		m_blackWinningPiece = new ImageIcon(getClass().getResource(
				"PieceBlackFinal.png"));
		m_whiteToBlackPiece = new ImageIcon(getClass().getResource(
				"PieceWhiteToBlack75.gif"));
		m_blackToWhitePiece = new ImageIcon(getClass().getResource(
				"PieceBlackToWhite75.gif"));
		setTime(0);
		setTimerLabel();
		startTimer();
	 ...
	 * 
	 * @param game
	 *            The OthelloGame object to be set.
	 */
	 ...
	 * 
	 * @param i
	 *            The column position of the piece to be set.
	 * @param j
	 *            The row position of the piece to be set.
	 * @param colour
	 *            The colour of the piece to be set.
	 */
	 ...
	 * 
	 * @param String
	 *            [] players - takes in a string of players first two values
	 *            become player 1 and player 2
	 */
	 ...
	 * 
	 * @param x
	 *            If 0 return player one else player two.
	...
	/**
	 * This is the method for animating the move of a piece on the board of the
	 * Othello game.
	 * 
	 * @return moveMade -if the move was successful moveMade returns true and
	 *         false if not.
	 */
	...
	/**
	 * This is the method for displaying the feedback message if a player makes
	 * an incorrect move.
	 * 
	 * @param message
	 *            -the variable storing the text of the message.
	 */
	...
	/**
	 * This method creates the GUI and board, adds current score, checks to see
	 * what button is pushed and sends that to the action listener. This
	 * essentially sets the original board and then sends it to the action
	 * listener to do the rest.
	 */
	void creatingGui() {
		m_window = new JFrame("Othello                 "
				+ m_playerOne.getName() + " turn      " + m_playerOne.getName()
				+ " Score: " + getGame().getPlayer1Score() + "       "
				+ m_playerTwo.getName() + " Score: "
	...
	/**
	 * This method creates the menu and then adds it to the board. It adds the
	 * pause button, save game, and load previous game. These have no been fully
	 * implemented yet.
	 * 
	 * @return -It returns
	 */
	...
	public boolean displayPlayAgain(String message) {
		...
		if (reply == JOptionPane.YES_OPTION) {
			m_window.dispose();
			m_panel.removeAll();
			OthelloGame game = new OthelloGame(new HumanPlayer(
					m_playerOne.getName(), m_p1colour), new HumanPlayer(
					m_playerTwo.getName(), m_p2colour));
	...
	/**
	 * setups up the menu bar which has options to load a game, save a game,
	 * pause the game and start a new game. load,save, and pause have not been
	...
	private JMenuBar creatingMenu() {
		...
		saveGameButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				String saveGame = JOptionPane
						.showInputDialog("Enter save name:");
				Date dNow = new Date();
				...
				new OthelloGameSaver("saves\\othellosaves\\" + saveGame + "  ["
						+ timeStamp.format(dNow) + "]" + ".xml");
		...
		loadGameButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					fileExtension = saveFile.substring(
							saveFile.lastIndexOf(".") + 1, saveFile.length());

				}
				if (fileExtension.equals("xml")) { // checks if file selected is
													// an xml file
					if (saveFile != "") { // checks if the user selected cancel
											// instead
						new OthelloGameLoader("saves\\othellosaves\\"
								+ saveFile);
					}
				} else {
	...
	/**
	 * This is a separate class inside of OthelloGameGui that receives any
	 * action done on the board. If a button is pushed, it is sent here to
	 * perform some action.
	 * 
	 * This method is where the action is performed. It searches through the
	 * board and finds our which piece is clicked and then adds a piece to it.
	 * It also contacts the game to find out if it's a valid move and if it is
	 * which pieces to flip.
	 * 
		...
		public void mouseReleased(MouseEvent event) {
			while (!m_animationThread.isAlive()) {
				...
				if (playerHuman) {
					for (int y = 0; y < TOTALWIDTH; y++) {
						for (int x = 0; x < TOTALHEIGHT; x++) {
							updateTitle(getGame().getPlayerTurn()
									% TOTAL_PLAYERS == 0);
							// player1 moves
				...
				if (!playerHuman) {
					...
					if (getGame().getPlayerTurn() % TOTAL_PLAYERS == 0) {
						...
						if (m_playerTwo.getPlayerType().equals("ComputerEasy")) {
							Point p = ((OthelloEasyComputerPlayer) m_playerTwo)
									.makeAIMove(m_board);
							i = (int) p.getX();
							j = (int) p.getY();
						} else {
							Point p = ((OthelloHardComputerPlayer) m_playerTwo)
									.makeAIMove(m_board);
							i = (int) p.getX();
							j = (int) p.getY();
						}
	...
	/**
	 * This method is used by the players to perform a move according to mouse
	 * input.
	 * 
	 * @param x
	 *            The column position where a move is to be performed.
	 * @param y
	 *            The row position where a move is to be performed.
	 */
	 ...
	 * 
	 * @param boolean player - used to define who's turn it is if true then
	 *        displays the score and that it is player one's turn if false then
	 *        displays the score and that it is player two's turn
	 */
	private void updateTitle(boolean player) {
		if (player) {
			m_window.setTitle(("Othello                 "
					+ m_playerOne.getName() + "'s turn          "
					+ m_playerOne.getName() + " Score: "
					+ getGame().getPlayer1Score() + "	    "
					+ m_playerTwo.getName() + " Score: " + getGame()
					.getPlayer2Score()));
		} else {
			m_window.setTitle(("Othello                 "
					+ m_playerTwo.getName() + "'s turn          "
					+ m_playerOne.getName() + " Score: "
					+ getGame().getPlayer1Score() + "	    "
					+ m_playerTwo.getName() + " Score: " + getGame()
					.getPlayer2Score()));
	...
	/**
	 * This method validates player one's moves and then calls player1SwapPieces
	 * to swap the valid pieces
	 * 
	 * @param int x - x coordinate of point on board
	 * @param int y - y coordinate of point on board
	 * @return boolean - returns false if move could not be validated or that
	 *         they have no valid move to make.
	 */
	...
	/**
	 * This method validates player two's moves and then calls player2SwapPieces
	 * to swap the valid pieces
	 * 
	 * @param int x - x coordinate of point on board
	 * @param int y - y coordinate of point on board
	...
	/**
	 * endGame() used to figure out if the game has finished checks if either
	 * player has any valid moves if no valid moves for either player then the
	 * game will end next it checks if either player has no peices left if a
	 * player has no peices then the game will end finally checks if the board
	 * is full if the board is full then the game will end
	 */
	private void endGame() {
		...
		if (!getGame().m_board.anyValid(m_player1Colour)
			...
			if (getGame().getPlayer1Score() > getGame().getPlayer2Score()) {
				winner = PLAYER_ONE;
			} else if (getGame().getPlayer1Score() < getGame()
					.getPlayer2Score()) {
				winner = PLAYER_TWO;
			} else {
				winner = DRAW;
		...
		if (winner == PLAYER_ONE) {
			...
			displayWinner(m_playerOne.getName() + " and "
					+ m_playerTwo.getName());
	...
	/**
	 * @param OthelloPiece
	 *            colour - takes in the winning player's colour replaces the
	 *            player's pieces with the winning pieces.
	 */
					...
					for (int i = 0; i < TOTALHEIGHT; i++) {
						for (int j = 0; j < TOTALWIDTH; j++) {
							if (m_gridButtons[i][j].getIcon() == m_blackPiece
									|| m_gridButtons[i][j].getIcon() == m_whiteToBlackPiece) {
								m_gridButtons[i][j]
										.setIcon(m_blackWinningPiece);
							}
						}
					}
				}
	...
	public static void main(String[] args) {
		...
		OthelloGameGUI displayExample = new OthelloGameGUI("Othello", WIDTH,
				HEIGHT, playerOne, playerTwo);
